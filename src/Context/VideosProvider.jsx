...
export const VideosProvider = ({ children }) => {
  ...
  const postData = async (newData) => {
    ...
    try {
      ...
      const upData = {
        ...state,
        videos: [...state.videos, { ...newData, id: uuidv4() }],
      };
  ...
  const updateData = async (updatedData, id = null) => {
   ...
   (id!==null)&& (data= state.videos.filter((item)=>item.id!==id))
      
   const dataUpdate= [...data,{...updatedData,id:id}]
    ...
    try {
      const response = await fetch(`${urlJsonBin}${tokenVideo}`, {
        ...
        body: JSON.stringify({ videos: dataUpdate }),
      });
        getData();
      dispatch({ type: ACTIONS.SUCCESS });
  ...
  const deleteData = async (id) => {
    dispatch({ type: ACTIONS.MAKE_REQUEST });
    try {
      const deleteItem = state.videos.filter((item) => item.id !== id);
